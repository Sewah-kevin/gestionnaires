...
if (isset($_POST['login'])) {
    ...
    if (!$nom && !$password) {
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            $_SESSION['nomGest'] = $user['nomGest'];
            $_SESSION['id'] = $user['idGest'];
...
if (isset($_POST['add_room'])) {
    ...
    if ($room_no != '') {
        ...
        if (mysqli_num_rows(mysqli_query($connection, $sql)) >= 1) {
            $response['done'] = false;
            $response['data'] = "La chambre n'existe pas encore !";
        } else {
            ...
            if ($result) {
                $response['done'] = true;
                $response['data'] = 'Chambre ajoutée avec succès !';
            } else {
                $response['done'] = false;
        ...
        $response['done'] = false;
        $response['data'] = "Veuillez entrer le numéro de chambre !";
    }
...
if (isset($_POST['edit_room'])) {
    ...
    if ($room_no != '') {
        ...
        if ($result) {
            $response['done'] = true;
            $response['data'] = 'Modification réussie !';
        } else {
            $response['done'] = false;
            $response['data'] = "Erreur de modification !";
        }
    ...
    if ($customer_result) {
        ...
        if ($booking_result) {
            ...
            if (mysqli_query($connection, $room_stats_sql)) {
                $response['done'] = true;
                $response['data'] = 'Réservation effectuée !';
            } else {
                $response['done'] = false;
                $response['data'] = "Erreur !";
            }
...
if (isset($_POST['check_in_room'])) {
    ...
    if ($booking_id != '') {
        ...
        if ($result) {
            ...
            $response['done'] = false;
            $response['data'] = "Probleme dans le payement !";
        }
    } else {
        $response['done'] = false;
        $response['data'] = "Erreur de réservation !";
    }
    echo json_encode($response);
}
...
if (isset($_POST['check_out_room'])) {
    ...
    if ($booking_id != '') {
        ...
        if ($remaining_price == $remaining_amount) {
            ...
            $response['done'] = false;
            $response['data'] = "Veuillez entrer le payement !";
        }
    } else {
        $response['done'] = false;
        $response['data'] = "Erreur !";
